...
const TodoList = () => {
    ...
    const todoTasks = async () =>{
      await axios.get('http://localhost:12345/todo/')
      .then(response => {
        console.log(response,":Cannot read properties of null (reading 'push')");
        if(response.data) {
  ...
  return (
    <>
        <div className='header'>
            <h1>Todo App</h1>
            <button className='btn_primary' onClick = {() => setModal(true)} style={{"cursor" : "pointer"}} >Create Task</button>
        </div>
        <div className='task-container'>
        {taskList && taskList.map((obj, index) => <Card key={obj.Id} taskObj={obj} index ={index} del={deleteTask} updatedTask={updateTask}/>)}
        </div>
